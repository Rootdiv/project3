...
session_start();
if (!empty($_SESSION['basket']) && !empty($_POST['amount']) && !empty($_POST['first_name']) &&
  !empty($_POST['last_name']) && !empty($_POST['address']) && !empty($_POST['city']) && !empty($_POST['postcode']) &&
  !empty($_POST['phone']) && !empty($_POST['email']) && !empty($_POST['total'])) {
  ...
  foreach ($all_goods as $elem) {
    ...
    $article_arr[] = $item->getField('article');
  ...
  $article = json_encode($article_arr, JSON_UNESCAPED_UNICODE);
  ...
  array_unshift($arr_values, "$product_name", "$article", "$size", "$quantity");
  array_unshift($arr_fields, 'product_name', 'article', 'sized', 'quantity');
  if (!$order_base->createLine($arr_fields, $arr_values)) {
    exit('Произошла системная ошибка');
  }
  ...
  $article_tm = 'Артикул: ' . str_replace('","', ', ', trim($article, '["]'));
  $size_tm = 'Размер: ' . $size;
  ...
  if (isset($mail_orders_tm) == 1) {
    $mail_error = 'Письмо не доставлено, произошла ошибка.';
  } else {
    $mail_error = '';
  }
