...
class FormValidator extends Validator {
  ...
  public function validate():array{
    ...
    if(isset($req['login'])) {
      ...
      if($mail){
        if(!preg_match($pattern_mail, $req['login'])) $this->addError('Недопустимый адрес E-mail');
        ...
        if(!preg_match('/^[a-z0-9]{4,15}$/isu', $req["login"])) $this->addError('Логин '.$not_str);
        $nameLen = mb_strlen($req['login']);
    ...
    if(isset($req['password'])){
      if(!preg_match('/^[a-z0-9\+\-\#\%\!]{5,17}$/isu', $req['password'])) $this->addError('Пароль '.$not_str);
    ...
    if(isset($req['username'])){
      if(!preg_match('/^[а-яё\-\s]{2,20}$/isu', $req['username'])) $this->addError('Имя '.$not_str);
    ...
    if(isset($req['email'])){
      if(!preg_match($pattern_mail, $req['email'])) $this->addError('Недопустимый адрес E-mail');
    ...
    if(isset($req['table_id'])){
      if($req['table_id'] == 0 || !preg_match($pattern_num, $req['table_id'])) $this->addError('Ошибка');
    }
    ...
    if(isset($req['product_id'])){
      if($req['product_id'] == 0 || !preg_match($pattern_num, $req['product_id'])) $this->addError('Ошибка');
    }
    if(isset($req['title'])){
      if(!preg_match('/^[a-zёа-я\-\s]{2,150}$/isu', $req['title'])) $this->addError('Название '.$not_str);
    ...
    if(isset($req['article'])){
      if(!preg_match($pattern_num, $req['article'])) $this->addError('Артикул может быть только цифровым');
    ...
    if(isset($req['description'])){
      if(!preg_match('/^[а-яё\-\.\,\s\!]{2,2000}$/isu', $req['description'])) $this->addError('Описание '.$not_str);
    ...
    if(isset($req['category'])){
      ...
      if($nameLen < 0 || $nameLen > 2) $this->addError($err_str);
    }
    ...
    if(isset($req['sized'])){
      if(!preg_match($pattern_num, $req['sized'])) $this->addError('Нужно указать размер цифрами');
    ...
    if(isset($req['quantity'])){
      ...
      for($i = 0; $i < $quantity; $i++){
        if($req['quantity'][$i] == 0 || !preg_match($pattern_num, $req['quantity'][$i])) $this->addError($err);
    ...
    if(isset($req['amount'])){
      if(!preg_match($pattern_num, $req['amount'])) $this->addError($err);
      $nameLen = mb_strlen($req['amount']);
      if($nameLen < 4 || $nameLen > 20) $this->addError($err_str);
    }
    if(isset($req['delivery'])){
      if(!preg_match($pattern_num, $req['delivery']) && $req['delivery'] !== 500 && $req['delivery'] !== 250 && $req['delivery'] !== 1000) $this->addError($err);
      $nameLen = mb_strlen($req['delivery']);
      if($nameLen < 3 || $nameLen > 4) $this->addError($err_str);
    }
    if(isset($req['first_name'])){
      if(!preg_match('/^[а-яё\-\s]{2,50}$/isu', $req['first_name'])) $this->addError('Имя '.$not_str);
    ...
    if(isset($req['last_name'])){
      if(!preg_match('/^[а-яё\-\s]{2,50}$/isu', $req['last_name'])) $this->addError('Фамилия '.$not_str);
    ...
    if(isset($req['address'])){
      if(!preg_match('/^[а-яё\-\.\s\d]{2,100}$/isu', $req['address'])) $this->addError('Адрес '.$not_str);
    ...
    if(isset($req['city'])){
      if(!preg_match('/^[а-яё\-\s]{2,50}$/isu', $req['city'])) $this->addError('Название города '.$not_str);
    ...
    if(isset($req['postcode'])){
      if(!preg_match('/^[0-9]{1,6}$/isu', $req['postcode'])) $this->addError('Индекс может быть только цифровым');
      $nameLen = mb_strlen($req['postcode']);
      if($nameLen < 1 || $nameLen > 6) $this->addError('Индекс не может превышать 6 цифр');
    }
    if(isset($req['phone'])){
      if(!preg_match('/^[+]{1}[0-9]{11}$/isu', $req['phone'])) $this->addError('Недопустимый формат номера');
      $nameLen = mb_strlen($req['phone']);
      if($nameLen < 1 || $nameLen > 12) $this->addError('Номер не может превышать 12 символов');
    }
    if(isset($req['total'])){
      if(!preg_match($pattern_num, $req['total'])) $this->addError($err);
      $nameLen = mb_strlen($req['total']);
      if($nameLen < 4 || $nameLen > 20) $this->addError($err_str);
    }
    if(isset($req['payment'])){
      if(!preg_match('/^[a-z\sа-я.]{2,50}$/isu', $req['payment'])) $this->addError($err);
    ...
    if(isset($req['fio'])){
      if(!preg_match('/^[a-zёа-я\-\s]{2,50}$/isu', $req['fio'])) $this->addError('Поле ФИО '.$not_str);
    ...
    if(isset($req['text'])){
      if(preg_match('/[\$\[\]*{}]+/isu', $req['text'])) $this->addError('Сообщение '.$not_str);
